...
__fastcall TDL::TDL(TComponent* Owner) : TForm(Owner) {}
...
bool TDL::GetFile(String& url, String& filename)
{
    ...
    try
    {
        ...
        filename = TPath::Combine(TPath::GetDownloadsPath(), basefile);
...
void __fastcall TDL::ReceiveDataEvent(
    TObject* const Sender, __int64 AContentLength, __int64 AReadCount, bool& Abort)
{
    if (Abort == false)
    {
